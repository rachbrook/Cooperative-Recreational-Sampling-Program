...
coop <- read_sheet("https://docs.google.com/spreadsheets/d/1_ydIN0FOKdNB5z35uV0qqcNfAGHdt1HGheUmDc5B1LI/edit?gid=1622078476#gid=1622078476", sheet = "Merged Data")
...
#District 1 - South
Vessel %in% c("Betty-O","New Del Mar","Eldorado","Amigo","(Newport) Patriot","Western Pride","Thunderbird","Fury","Sum Fun","Outer Limits","Premier","Malihini" ) ~ "District 1 - South",

# District 2 - Channel  
Vessel %in% c("Endeavor","Island Spirit","Coral Sea","Stardust","Mirage") ~ "District 2 - Channel",

# District 3 - Central    
Vessel %in% c("Infinity","Legacy","Big Anchovy","Kahuna","Last Meal","The Marianne","Black Pearl","Fiesta","(Avila) Patriot") ~ "District 3 - Central",  

# District 4 - San Francisco    
Vessel %in% c("Dragonfly","Big Eye","Sorta Salty","New Sea Angler","Salty Lady","Lovely Martha","Sea Wolf","Queen of Hearts","New Captain Pete","Riptide") ~ "District 4 - San Francisco",

# District 5 - Wine   
Vessel %in% c("Sea Hawk","Saylor Ann","Kyndall Lynn","Telstar") ~ "District 5 - Wine",

# District 6 - Redwood    
Vessel %in% c("Onyx","Out Of The Box","New Reel Steel","Scrimshaw","Fishy Business") ~ "District 6 - Redwood",  # Changed from `==` to `%in%`
...
## Summary of cooperative data by CRFS district
...
species_summary_wide <- coop %>%
  ...
  group_by(Scientific_Name, Species_Name, year) %>%       
  summarise(n_fish = n(), .groups = "drop") %>%
  pivot_wider(
    names_from = year,
    values_from = n_fish,
  ...
  select(Scientific_Name, Species_Name, `2022`, `2023`, `2024`, `2025`) %>%
  mutate(Total = `2022` + `2023` + `2024` + `2025`) %>%
...
datasummary_crosstab(CRFS_District ~ Year, statistic = 1 ~ N, title = 'Table 1: Summary of groundfish sampled in 2022-2025', data = coop)

datasummary_crosstab(Species_Name ~ Year,statistic = 1 ~ N, title = 'Table 1: Summary of species sampled in 2022-2025', data = coop)
...
GSPT_filtered_data <- Coop_Data %>%
  filter(Species_Name == target_species)
...
GSPT_all_data <- GSPT_filtered_data %>%
...
# Save File
write.csv(GSPT_all_data, "GSPT-All-Data-RB20250312.csv", row.names = FALSE)
